...
async fn main() {
    ...
    let path = std::path::PathBuf::from(args.get(1).expect("usage: gensheet <video> [cols] [rows] [q] [mb]"));
    ...
    match run_job(&path, &config, &fonts, &ctl).await {
        Ok((meta, outcome)) => {
            eprintln!("\n{}×{} {:.1}s {} fps {}", meta.width, meta.height, meta.duration_s, meta.fps, meta.codec);
            for a in outcome.artifacts {
                println!("{} · {:.2} MB{}", a.path, a.bytes as f64 / 1e6, if a.degraded { " · degraded" } else { "" });
            }
